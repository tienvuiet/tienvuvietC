...
void addCategory() {
    ...
    for (i = 0; i < numCategories; i++) {
        ...
        while (!isValid) {
            ...
            c.name[strcspn(c.name, "\n")] = '\0'; //tra ve vi tri xuat hien dau tien cua ki tu \n thay no bang \0
            if (strlen(c.name) == 0) {
    ...
    for (i = 0; i < count; i++) {
        if (categories[i].id == idToDelete) {
            ...
            if (confirmation == 'y' || confirmation == 'Y') {
                FILE *newFile = fopen("categories_temp.bin", "wb");
                if (newFile == NULL) {
                ...
                // Xoa cac san pham thuoc danh muc
                FILE *productFile = fopen("products.bin", "rb");
                if (productFile == NULL) {
                    printf("Error: Unable to open product file.\n");
                    return;
                }
                FILE *tempProductFile = fopen("products_temp.bin", "wb");
                if (tempProductFile == NULL) {
                    printf("Error: Unable to open temporary product file.\n");
                    fclose(productFile);
                    return;
                }
                Product product;
                while (fread(&product, sizeof(Product), 1, productFile)) {
                    if (product.categoryId != idToDelete) {
                        fwrite(&product, sizeof(Product), 1, tempProductFile);
                    }
                }
                fclose(productFile);
                fclose(tempProductFile);
                remove("products.bin");
...
void searchCategoryByName() {
    ...
    while (fread(&c, sizeof(Category), 1, file)) {
        ...
        //tro kieu den vi tri trong chuoi noi tim thay chuoi con searchName trong chuoi lon c.name or NULL neu
...
void sortCategoriesByName() {
    ...
    Category categories[100]; // Mang tam de luu cac danh muc, giup thao tac trong file toi uu hon, cu mo va doc trg file nhieu lan
    int count = 0;           
    while (fread(&categories[count], sizeof(Category), 1, file)) {
        count++;
    }
...
void addProduct() {
    ...
    Category category;//khai bao bien co kieu du lieu Category mang tat ca thuoc tinh C
    int maxIdLength = 2;       
    int maxNameLength = 4;     
...
void viewProductsByCategory() {
    ...
    while (fread(&category, sizeof(Category), 1, categoryFile)) {
        ...
        for (i = 0; i < tableWidth; i++) printf("=");
        printf("\n");
        printf("| %-*s | %-*s | %-*s | %-*s |\n", maxIdLength, "Product ID", maxNameLength, "Name", maxPriceLength, "Price", maxQuantityLength, "Quantity");
        for (i = 0; i < tableWidth; i++) printf("=");
        printf("\n");
        int found = 0;
        rewind(productFile); 
        while (fread(&product, sizeof(Product), 1, productFile)) {
            if (product.categoryId == category.id) {
                found = 1;
                printf("| %-*d | %-*s | %-*.2f | %-*d |\n", maxIdLength, product.id, maxNameLength, product.name, maxPriceLength, product.price, maxQuantityLength, product.quantity);
            }
        }
        if (!found) {
            printf("| %-*s |\n", tableWidth - 4, "No products found in this category.");//chinh -4 moi vua khong biet tai sao
        }
        for (i = 0; i < tableWidth; i++) printf("=");
        printf("\n");
    }
...
void editProductInformation() {
    ...
    printf("\n");
    for (i = 0; i < count; i++) {
        printf("                              | %-*d | %-*s | %-*.2f | %-*d |\n",maxIdLength, products[i].id,maxNameLength, products[i].name,maxPriceLength, products[i].price,maxQuantityLength, products[i].quantity);
    }
    printf("                              ");
    for (i = 0; i < tableWidth; i++) printf("=");
    printf("\n");
    ...
    for (i = 0; i < count; i++) {
        if (products[i].id == productId) {
            ...
            printf("Enter new quantity (or enter 0 to keep current): ");
            int newQuantity;
            scanf("%d", &newQuantity);
            fflush(stdin);
            if (newQuantity > 0) {
